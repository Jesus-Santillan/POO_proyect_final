...
class ProductosView:
    def __init__(self, username):
        ...
        # Si ya existe una ventana Tk principal, abrir como Toplevel para evitar múltiples roots
        self.root = tk.Toplevel() if tk._default_root else tk.Tk()
        self.root.title(f"Productos - Usuario: {username}")
        self.root.geometry("900x520")
        self.root.resizable(True, True)

        # Confirmar cierre de ventana con el mismo handler
        self.root.protocol("WM_DELETE_WINDOW", self.cerrar_sesion)

        self.crear_elementos()
        self.mostrar_productos()

        # Solo ejecutar mainloop si creamos la ventana principal (Tk), no si es Toplevel
    ...
    def crear_elementos(self):
        ...
        # Evitar crear una nueva instancia de la misma vista desde sí misma (recursión).
        # En esta vista el botón sirve para refrescar la lista de productos.
        tk.Button(frame_botones, text="Refrescar productos", width=18, command=self.mostrar_productos).pack(side="left", padx=6, pady=6)
    ...
    def abrir_productos(self):
        if ProductosView:
            ...
            messagebox.showinfo("No disponible", "La vista de productos no está disponible.")
    ...
    def actualizar_producto(self):
        ...
        entry_marca.pack(pady=4, fill="x", padx=10)
